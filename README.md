...
2. Open the `.env` file and add your custom API configurations (e.g., for local/OpenAI-compatible endpoints):
```env
...
* Install all necessary dependencies from `my_agent/requirements.txt`.
...
└── my_agent/             # Main working directory for the AI Agent
    ...
    └── requirements.txt  # Explicit Python dependencies for this agent project
