...
class ModelMenuCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        if text.startswith("/model") and self.provider_manager:
            yield from self._get_model_completions(text)
            return
    ...
    def _get_model_completions(self, text):
        parts = text.split()
        ...
        if len(parts) == 2 and text.endswith(" "):
            ...
            if provider_name == "cancel": return
...
def _handle_enter_key(event):
    ...
    if stripped_text == "/model /back" or (stripped_text.endswith("/back") and stripped_text.count(" ") == 2):
        buffer.text = "/model "
...
def create_cli_session(provider_manager=None) -> PromptSession:
    command_completer = ModelMenuCompleter(provider_manager)
    kb = KeyBindings()
    ...
    def _(event):
        buffer = event.app.current_buffer
        if buffer.complete_state: buffer.complete_previous()
        else: buffer.auto_up()

    @kb.add("down")
    def _(event):
        buffer = event.app.current_buffer
        if buffer.complete_state: buffer.complete_next()
        else: buffer.auto_down()
